  ...
  useEffect(() => {
    if (isOpen) {
      ...
      if (focusables?.length) {
        setFirstFocusableElement(focusables[0] as HTMLElement)
        setLastFocusableElement(focusables[focusables.length - 1] as HTMLElement)
      ...
      {isOpen && (
        <>
          <motion.div
            ...
            className="fixed inset-0 h-full z-50 w-full bg-transparent"
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
...
function DialogClose({ children, className }: DialogCloseProps) {
  ...
  return (
    <button onClick={() => setIsOpen(false)} className={className}>
      {children || <XIcon />}
    </button>
  )
}
